    ...
    <div className="d-flex justify-content-center align-items-center  vh-100" style={{backgroundColor:"#f3f9d2"}}>
      <div className="p-3 rounded w-1/3" style={{backgroundColor:"#cbeaa6"}}>
        <form action="" onClick={handleSubmit}>
          {!status ? (
            <button type="submit" className="btn " style={{backgroundColor:"#78a1bb", color:"white"}}>
              GET All Record
            </button>
          ) : (
            ""
        ...
        {status ? (
          <Link
            to="/"
            type="submit"
            className="btn  border w-50 rounded-10 m-10 mt-2"  style={{backgroundColor:"#78a1bb", color:"white"}}
          >
            Back
          </Link>
        ) : (
